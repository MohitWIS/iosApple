...
init: function() {
    try {
        this.coursesDB = window.sqlitePlugin.openDatabase({
            name: "ETEACHER.db",
            location: 'default'
        });
        //this.coursesDB = openDatabase("Database", "1.0", "ETEACHER", 10000);
        this.coursesDB.transaction(function(tx) {
                                   tx.executeSql("CREATE TABLE IF NOT EXISTS ETEACHER_COURSE (id unique, courseid, title, status, coursestatusid, buypackage, freeaccess)");
                                   });
    ...
    try {
        this.coursesDB.transaction(function(tx) {
                                   tx.executeSql("SELECT * FROM ETEACHER_COURSE", [], function(tx, results) {
                                                 ...
                                                 for (var i = 0; i < len; i++) {
                                                 var savedCourse = new Course(results.rows.item(i).courseid, results.rows.item(i).title, results.rows.item(i).status, results.rows.item(i).coursestatusid, results.rows.item(i).buypackage,results.rows.item(i).freeaccess);
    ...
    try {
        ...
        coController.coGetCourses(function(ret) {
                                  ...
                                  if (coursesObj.UserPosition !== undefined) {
                                      userPositionObj = coursesObj.UserSyncPosition;
                                  ...
                                  course = new Course(courseobj.CourseId, courseobj.Title, courseobj.Status, courseobj.CourseStatusId, courseobj.Display, courseobj.Accessible, courseobj.InAccessibleFunction, courseobj.BuyPackage,courseobj.FreeAccess);
    ...
    try {
        ...
        if (this.courses.length > 0) {
            ...
            $.each(this.courses, function(i, course) {
                   ...
                   if (course.accessible) {
                   courseStr += "<div id='" + courseid + "' data-role='collapsible' class='coursesli " + course.inaccessiblefunction +" "+ course.buypackage+ " " + course.freeaccess+ "' data-theme='h' data-content-theme='h' data-iconpos='right'>";
...
save: function(course) {
    try {
        ...
        if (course.buypackage == undefined) {
            course.buypackage = "";
        }
        if (course.freeaccess == undefined) {
            course.freeaccess = "";
        }
        this.usersDB.transaction(function(tx) {
                                 tx.executeSql("DELETE * FROM ETEACHER_COURSE WHERE courseid=" + course.courseid);
                                 tx.executeSql("INSERT INTO ETEACHER_COURSE(courseid, title, status, coursestatusid,buypackage,freeaccess) VALUES (course.courseid, course.title, course.status, course.coursestatusid, course.buypackage,course.freeaccess)");
                                 });
    } catch (e) {
...
var Course = function(courseid, title, status, coursestatusid, display, accessible, inaccessiblefunction, buypackage, freeaccess) {
    try {
        this.init(courseid, title, status, coursestatusid, display, accessible, inaccessiblefunction, buypackage, freeaccess);
    } catch (e) {
        errorHandler("Course", e);
    }
};
Course.prototype = {
init: function(courseid, title, status, coursestatusid, display, accessible, inaccessiblefunction, buypackage, freeaccess) {
    try {
        this.modulegroups = [];
        ...
        if (buypackage == undefined) {
            buypackage = false;
        }
        if (freeaccess == undefined) {
            freeaccess = false;
        }
        ...
        if (buypackage) {
            buypackage = "assessmentPackagelist";
        }
        if (!freeaccess) {
    ...
    try {
        ...
        if (userModules != undefined) {
            ...
            if (userModules.length == undefined || userModules.length == 0) {
                modObj = userModules;
                module = new Module(this.courseid, modObj.BaseModuleId, modObj.Title, modObj.Status, modObj.BaseModuleTypeId, modObj.ModuleOrder, modObj.Enabled, modObj.Action, modObj.EventName, modObj.ActionId,modObj.actionId,modObj.PracticeIsOptional,modObj.otaTypeId, modObj.EventId, modObj.AssessmentScheduleId, modObj.AssessmentBookingId, modObj.Display, modObj.Accessible, modObj.InAccessibleFunction, modObj.ModuleDescription, "", modObj.ModuleUrl, modObj.ModuleGrouping, modObj.IsRequiresBooking, modObj.FirstTimePackageAccess, modObj.Price, modObj.IsOptional);
        ...
        for (var x= 1; x < 5; x++){
            ...
            if( modules!==undefined && modules.length > 0){
                ...
                $.each(modules, function(i, module) {
                       ...
                       title = title.substring(0, 29) + "...";
                       singleClass=" groupsingleline";
                       }else if(title.length < 29 && tablet===false && iphone5===true){
                           title = title.substring(0, 29) + "...";
                           singleClass=" groupsingleline";
                       }else if(title.length < 92 && tablet===true){
                           title = title.substring(0, 29) + "...";
                       ...
                       modlistStr += "<li id='" + id + "' class='moduleli " + module.inaccessiblefunction + " paymentvalues-price-" + module.basemoduleid + "-" + module.price + "-0-0-0-0-0-2-0-modulePayment-" + module.isOptional + "-" + module.status;
                       modlistStr += "'>";
                       modlistStr += "<div class='limodtitleactive'><div class='statuslock " + classStatus + "'></div>";
...
init: function(courseid, basemoduleid, title, status, basemoduletypeid, moduleorder, enabled, action, eventname, actionid,actionIdNew,practiceisoptional,otaTypeId, eventid, assessmentscheduleid, assessmentbookingid, display, accessible, inaccessiblefunction, description, thumbnail, moduleurl, modulegrouping, isRequiresBooking, firstTimePackageAccess, price, isOptional) {
    try {
        ...
        if (inaccessiblefunction.indexOf("prevModulesIncomplete") > -1 && status == "TAPRequired") {
            inaccessiblefunction = "prevmodulesincomplete";
            status = "In Progress";
        }else {
            if (inaccessiblefunction.indexOf("modulePaymentRequired") > -1 && status != "TAPRequired") {
                inaccessiblefunction = "modulepaymentrequired";
            } else if(status == "TAPRequired"){
                inaccessiblefunction = "TAPRequired";
            }else {
...
refreshLessons: function(returnFunction) {
    try {
        var that = this;
        coController.coGetLessonStructure(this.basemoduleid,this.courseid, function(ret) {
                                          if (ret == undefined) {
                                          msgTitle = resources.connError;
                                          msgBtnValue = resources.btnOk;
                                          msgStr = resources.connectionFail;
                                          ...
                                          that.theoryModule = new TheoryModule(that.courseid, that.basemoduleid, theoryModObj.HasPodcast, theoryModObj.PDFUrl, theoryModObj.PodcastUrl, theoryModObj.LSURLIsYUDU,theoryModObj.IsShopURL, theoryModObj.YUDUID, theoryModObj.BuyPackage);
                    ...
                                } else {
                        ...
                                        for (var i = 0; i < this.assessmentschedules.length; i++) {
                                              if(GetAssessmentSchedulesParse.PayToBook == true && GetAssessmentSchedulesParse.CourseOverridePayToBook == false && GetAssessmentSchedulesParse.ModulePayToBookStatusID == 2){
                                ...
                                                    schedulesStr += "<li class='scheduleitem  " + schedulesId + "' data-theme='h'><div style='padding-bottom: 8px;' class='scheduleitemmain'>";
                                                    schedulesStr += "<div class='scheduleitemhead'>";
                                                    //schedulesStr += this.title + ", " + schedule.starttime + ", " + schedule.venuename;
                                                    var title_booking;
                                                    if(schedule.venuename.length>=12){
                                                        title_booking = schedule.venuename.substring(0, 12) + "...";
                                                    }else{
                                                        title_booking = schedule.venuename;
                                                    }
                                                    schedulesStr += schedule.starttime + ", " + title_booking;
                                                    schedulesStr += "</div>";
                                                    schedulesStr += "<div class='bookbtn' style='width: 115px;margin-top: -8px;height: 39px;float:right;' id="+CalltoPay+"><a style='font-size: 15px;padding-top: 11px;' class='schedulebookbtn paynbook ";
                                ...
                                                    schedulesStr += " ui-btn ui-btn-h' data-theme='h' href='#' id='book-" + schedule.assessmentscheduleid + "-" + schedule.assessmentitemid + "'>" + resources.bookbtntxtPayToBook + "</a></div>";
                                                    schedulesStr += "<div class='viewbookbtn' style='width:70px !important;margin-top: -7px;height: 38px;padding-bottom: 6px !important;float:right;'><a style='background-color: #a46eab !important;font-size: 15px;padding-top: 11px;' class='viewschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='viewbook-" + schedule.assessmentscheduleid + "-" + schedule.assessmentitemid + "'>" + resources.detailsbtntxt + "</a></div>";
                                                    
                                                    schedulesStr += "</li>";
                                                  }
                            ...
                                              else if(GetAssessmentSchedulesParse.CourseOverridePayToBook == true){
                                ...
                                                    if (bkDate != schedule.scheduleddatestring) {
                                    ...
                                                        }
                                                    var schedulesId = "schedules-" + $.trim(highLightDates[(hd - 1)]);
                                                    schedulesStr += "<li style='max-height:38px;' class='scheduleitem  " + schedulesId + "' data-theme='h'><div class='scheduleitemmain'>";
                                                    schedulesStr += "<div class='scheduleitemhead'>";
                                                    var title_booking;
                                                    if(schedule.venuename.length>=12){
                                                        title_booking = schedule.venuename.substring(0, 12) + "...";
                                ...
                                                    schedulesStr += schedule.starttime + ", " + title_booking;
                                                    schedulesStr += "</div>";
                                                    schedulesStr += "<div class='bookbtn' style='width: 70px;margin-top: -8px;height: 39px;float:right;'><a style='font-size: 15px;padding-top: 11px;' class='schedulebookbtn ";
                                                    schedulesStr += schedule.restrictbooking===true ? 'restrictbooking' : '';
    ...
    try {
        ...
        for (var y = 0; y < this.assessmentschedules.length; y++) {
            ...
            if (scheduleBook.booking != undefined && scheduleBook.booking.assessmentbookingid > 0) {
                //alert("in true block: "+scheduleBook);
                ...
                var scheduleBookVenue = scheduleBook.venuename;
                var scheduleBookVenueTitle = scheduleBookVenue.substring(0, 25);
                schedulesStr += scheduleBook.starttime + ", " + scheduleBookVenueTitle;
                ...
                if (scheduleBook.booking.resultpending === false) {
                    if (scheduleBook.booking.allowcancel == true) {
                        ...
                        schedulesStr += "<div style='width: 70px;margin-top: -8px;height: 39px;float:right;' class='cancelbookbtn redColorButton'><a style='font-size: 15px;padding-top: 11px;' class='cancelschedulebookbtn redColorButton ui-btn ui-btn-h' data-theme='h' href='#' id='cancelbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "-" + scheduleBook.assessmentbookingid + "'>" + resources.cancelbtntxt + "</a></div>";
                        schedulesStr += "<div class='viewbookbtn' style='background-color: #a46eab !important;width:70px !important;margin-top: -7px;height: 38px;padding-bottom: 6px !important;float:right;'><a style='background-color: #a46eab !important;font-size: 15px;padding-top: 11px;' class='viewschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='viewbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "'>" + resources.detailsbtntxt + "</a></div>";
                        ...
                        if(scheduleBook.showattendbutton == true){
                            ...
                            schedulesStr += "<div id=hidebooking-"+scheduleBook.assessmentbookingid+" style='float:left;font-size: 0.8em;padding-top: 4%;letter-spacing: 3px;display:none;' class='"+showPassowrdStar+"'>******</div><img id='"+scheduleBook.assessmentbookingid+"' alt='"+scheduleBook.zoommeetingpassword+"' style='display:none;height: 21px;float: left;padding-top: 2.6%;' class='"+showPassowrd+"' src='css/client/images/passwordHide.png' >";
                            schedulesStr += "<div class='cancelbookbtnnew greenColorButton' style='width: 72px;margin-top: -8px;height: 39px;'><a style='font-size:15px;' class='attendschedulebookbtn greenColorButton ui-btn ui-btn-h' data-theme='h' href='#' id='"+scheduleBook.zoomjoinURL+"'>" + resources.AttendBtn + "</a></div>";
                            showPassowrd = "NotPassowrd";
                            showPassowrdStar = "NotStar";
                        }
                        else{
                            schedulesStr += "<div style='width: 70px;margin-top: -8px;height: 39px;float:right;' class='cancelbookbtn redColorButton'><a style='font-size: 15px;padding-top: 11px;' class='nocancelschedulebookbtn redColorButton ui-btn ui-btn-h' data-theme='h' href='#' id='cancelbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "-" + scheduleBook.assessmentbookingid + "'>" + resources.cancelbtntxt + "</a></div>";
                            schedulesStr += "<div class='viewbookbtn' style='background-color: #a46eab !important;width:70px !important;margin-top: -7px;height: 38px;padding-bottom: 6px !important;float:right;'><a style='background-color: #a46eab !important;font-size: 15px;padding-top: 11px;' class='viewschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='viewbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "'>" + resources.detailsbtntxt + "</a></div>";
                        }
                    ...
                    schedulesStr += "<div style='width: 37% !important;text-align: center;margin-left: 6% !important;font-size: 15px;margin-top: -7px;height: 27px;padding-top: 13px;float: right;' class='cancelbookbtn greyColorButton'>"+ resources.bookingAwaitingResult +"</div></li>";
...
var TheoryModule = function(courseid, basemoduleid, haspodcast, pdfurl, podcasturl, LSURLIsYUDU,IsShopURL, YUDUID, BuyPackage) {
    try {
        this.init(courseid, basemoduleid, haspodcast, pdfurl, podcasturl, LSURLIsYUDU,IsShopURL, YUDUID, BuyPackage);
    } catch (e) {
        errorHandler("TheoryModule", e);
    }
};
TheoryModule.prototype = {
init: function(courseid, basemoduleid, haspodcast, pdfurl, podcasturl, LSURLIsYUDU, IsShopURL,YUDUID,BuyPackage) {
    try {
...
getWorksheetViewContent: function(completedResult, returnFunction) {
    try {
        ...
        for (var i = 0; i < questionlist.length; i++) {
            ...
            wsStr += "</div></div>";
            wsStr += "<div class='wsrow wsqnormal'><div style='padding-top:6px;' class='wsqno'>Correct<br>Answer</div>";
            wsStr += "<div class='wsq wsqnormal' style='padding-top:10px;'><span style='display: inline-block;border: 1px #2e303c solid;padding: 5px;background-color: #f4eeea;margin-left: 1px;width: auto;'>" + question.correctanswer + "</span></div></div>";
    ...
    try {
        ...
        coController.coSaveWorksheetData(returnWorksheet, function(ret) {
                                         ...
                                         that.result = ret.Result;
                                             that.isThirdWorksheetCompleted = ret.isThirdWorksheetCompleted;
                                             window.localStorage.setItem("isThirdWorksheetCompleted", ret.isThirdWorksheetCompleted);
...
function setModuleStatusClass(status, accessible, divider) {
    ...
    if (status == courseStatus.InProgress && accessible===true) {
        ...
        classStatus = "status2 statusns";
    } else if (status == "TAPRequired" && accessible===false) {
        classStatus = "status4 statusns";
        ...
        if (status == courseStatus.Exempt && accessible===false) {
            ...
            if (status == courseStatus.Completed) {
                classStatus = "status1 statuscp 2";
            } else {
